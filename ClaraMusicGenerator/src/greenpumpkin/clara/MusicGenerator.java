...
public class MusicGenerator {
	...
	public MusicGenerator() {
		...
		//These 2 if/else statements will decide what scale to use, whether it be major or minor
		if(Math.random()<0.9)
		...
		System.out.println(chords.progression.size() + " " + (chords.progression.toString()));
	...
	public void createTest() {
		player.play("T120 V0 60w 55w 64w 60w V1 36w+48w 36w+48w 36w+48w 36w+48w");
		 ...
		 ParserListenerAdapter adapter = new ParserListenerAdapter();
		 parser.addParserListener(adapter);
		 Pattern bob = new Pattern();
		 bob.add("C7w + C6w");
		 bob.setTempo(120);
		 System.out.println(bob.toString());
		 OutputStream file = null;
		try {
			String filePath = new File("assets/bob.mid").getAbsolutePath();
			file = new FileOutputStream(filePath);
		} catch (FileNotFoundException e1) {
			e1.printStackTrace();
		}
		 try {
			MidiFileManager.savePatternToMidi(bob.getPattern(), file);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
